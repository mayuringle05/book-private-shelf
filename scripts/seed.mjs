...
if (!process.env.DATABASE_URL && fs.existsSync(".env")) loadEnvFile(".env");
const { Client } = pg;
const url = process.env.DATABASE_URL;
if (!url) throw new Error("DATABASE_URL is required. Create .env from .env.example or export DATABASE_URL before seeding.");
const ssl = process.env.DATABASE_SSL === "true" ? { rejectUnauthorized: false } : false;
const client = new Client({ connectionString: url, ssl });
await client.connect();

const chapterSchema = { chapter_number:"number", title:"string", hook:"string", sections:[{heading:"string",body:"string"}], examples:["string"], exercise:{prompt:"string",reflection_questions:["string"]}, summary:"string", word_count:"number" };
...
try {
  ...
  for (const b of books) {
    ...
    for (const u of units) await client.query(`INSERT INTO book_units (book_id, unit_key, unit_type, unit_number, sequence, title_hint, required_fields, output_schema, min_word_count)
